...
const date=s=>new Date(s).toLocaleString('ru-RU',{timeZone:'Europe/Moscow',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'})+' МСК';
export class Billing{
 ...
 async menu(job,user,buy=false){
  if(!buy)await this.radar.insights?.event(user,'upgrade_open',job.id);
  ...
  if(a.tier==='paid')return this.radar.reply(job,user,`💎 <b>Полный портфель открыт</b>\n\nДоступ до ${date(a.paid_until)}.\n${sub?.renew_enabled?'Следующее списание — 290 ₽: '+date(sub.next_charge_at||a.paid_until)+'.':'Автопродление отключено. После оплаченного периода останутся три актива.'}\n\n/unsubscribe — отключить автосписания\n/paysupport — помощь с оплатой`);
  const text=`💎 <b>Portfolius · 290 ₽ / неделю</b>\n\n• Новости и котировки всего портфеля\n• Прогнозы и ориентиры всех активов, где есть открытые данные\n• Структура и концентрация вложений\n• Календарь событий и ожидаемых выплат\n• Итоги недели и архив на 90 дней\n• До 3 запросов к ИИ в день по готовым данным\n\n${offer.text}\n\n<b>Условия оплаты</b>\nПервая неделя оплачивается сейчас. Оставшиеся бесплатные дни сохраняются. Далее — 290 ₽ каждые 7 дней с привязанной карты через Т‑Банк.\n\n/unsubscribe отключает следующие списания. Без оплаты остаются сводки по трём активам и прогнозы по одному крупнейшему. /terms — полные условия.`;
