...
import {dailyResearchKey,quoteCacheKey,newsDay} from './daily.js';
...
export class Insights{
 ...
 async sendReport(job,user,report,section='summary',page=0,{automatic=false}={}){
  ...
  await this.db.post('pr_outbox',{dedup_key:`${job.id}:report:${report.id}:${section}:${page}`,user_id:user,body:{...body,_portfolius:{report_id:report.id,section,page,automatic,kind:report.kind}}},{on_conflict:'dedup_key'},'resolution=ignore-duplicates,return=minimal');
 }
 ...
 async delivered(row){
  const g=row.body._portfolius;if(g?.report_id&&g.automatic)await this.event(row.user_id,g.kind==='weekly'?'weekly_delivered':'report_delivered',g.report_id);
 }
